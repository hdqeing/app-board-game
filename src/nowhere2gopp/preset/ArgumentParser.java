...
public class ArgumentParser {
        ...
        try {
            while (index < args.length) {
                ...
                if (args[index].startsWith("--")) {
                    ...
                    index += 2;
                } else throw new ArgumentParserException("Error parsing: " + args[index]);
            }
    ...
    private void addFlag(final String flag) throws ArgumentParserException {
        ...
        if (params.containsKey(flag)) throw new ArgumentParserException("Param already exists: " + flag);
    ...
    private void addSetting(final String key, final String value) throws ArgumentParserException {
        // Check if a param with this name already exists
        if (params.containsKey(key)) throw new ArgumentParserException("Param already exists: " + key);

        if (value.startsWith("-")) throw new ArgumentParserException("Setting value wrong format: " + value);
    ...
    protected boolean getFlag(final String flag) throws ArgumentParserException {
        if (!params.containsKey(flag)) return false;

        Object o = params.get(flag);

        if (!(o instanceof Boolean)) throw new ArgumentParserException("This is not a flag");
    ...
    protected Object getSetting(final String key) throws ArgumentParserException {
        if (!params.containsKey(key)) throw new ArgumentParserException("Setting " + key + " not " + "defined");
    ...
    /**
     * Der "host" Schalter wird eingeschaltet wenn das Prozess als host dient.
     */
    public boolean isHost() throws ArgumentParserException {
        return getFlag("host");
    }

    public boolean isOnline() throws ArgumentParserException {
        return getFlag("online");
    }

    public boolean isRedTextEnabled() throws ArgumentParserException {
        return getFlag("redTextEnabled");
    }

    public boolean isBlueTextEnabled() throws ArgumentParserException {
        return getFlag("blueTextEnabled");
    }
}
